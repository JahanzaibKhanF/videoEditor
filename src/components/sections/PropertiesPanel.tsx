...
import { Scissors } from "@/utils/icons";
...
export default function PropertiesPanel() {
  ...
  const [bgRemovalClipId, setBgRemovalClipId] = useState<string | null>(null);
  const {
  ...
  return (
    <div className="flex flex-col h-full bg-studio-surface">
          ...
          if (!clip) {
            return (
              <div className="flex flex-col items-center justify-center py-14 gap-3 text-ink-faint">
                ...
                <p className="text-[12px] text-center font-medium text-ink-muted">Select a video clip<br />to add effects</p>
              </div>
            );
          }
          return (
            <div className="flex flex-col gap-3">
              <button
                onClick={() => setBgRemovalClipId(clip.id)}
                className="w-full flex items-center gap-3 p-3 rounded-xl text-left transition-transform hover:scale-[1.01]"
      ...
      {bgRemovalClipId && (() => {
        const targetClip = clipsDetails.find(c => c.id === bgRemovalClipId);
        if (!targetClip) return null;
        return <BackgroundRemovalModal clip={targetClip} onClose={() => setBgRemovalClipId(null)} />;
      })()}
    </div>
  );
}
