    ...
    files.forEach((file, index) => {
      if (file.type.startsWith("video/")) {
        ...
        imgEl.onload = () => {
          ...
          reader.onload = () => {
            newImages.push({ id, src: imgEl.src, image: file, opacity: 1, imageX: placementIndex * 40, imageY: placementIndex * 30, width: imgEl.width, height: imgEl.height, scaleX: .4, scaleY: .4, startTime: 0, endTime: totalTime, animation: "none" });
  ...
  if (activeTab === "effects") {
    ...
    <div className="flex flex-col h-full bg-studio-surface">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto scrollbar-thin p-3 flex flex-col gap-4">
        ...
        <div>
          <div className="text-[10.5px] font-bold uppercase tracking-[.7px] text-ink-muted mb-2 flex items-center gap-1.5">
            <Sparkles size={11} className="text-signal" /> AI Background Removal
          </div>
          <button
            onClick={() => selectedClip && setBgRemovalOpen(true)}
            disabled={!selectedClip}
            className="w-full flex items-center gap-3 p-3 rounded-xl text-left transition-transform enabled:hover:scale-[1.01] disabled:opacity-40 disabled:cursor-not-allowed"
            style={{ background: "linear-gradient(135deg,#8B5CFF 0%,#4C8CFF 100%)" }}
          >
            <div className="w-9 h-9 rounded-lg bg-white/20 flex items-center justify-center flex-shrink-0 relative">
              <Scissors size={16} color="white" />
              <span className="absolute -top-1.5 -right-1.5 text-[7px] font-black bg-white text-signal rounded-full px-1 py-px leading-tight">AI</span>
            </div>
            <div>
              <div className="text-[13px] font-bold text-white">Remove Background</div>
              <div className="text-[10.5px] text-white/75">Cut out the subject — real-time preview</div>
            </div>
          </button>
        </div>
