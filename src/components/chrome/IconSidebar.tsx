"use client";
import { IoTextOutline, GoFileMedia, GoSun, TbTransitionRight, FiLayers, CgTemplate, Scissors, Wand2 } from "@/utils/icons";
...
export default function IconSidebar({ activeTab, onTabChange }: Props) {
  return (
    ...
    >
      {GROUPS.map((group, gi) => (
        <div key={group.cap} className="flex flex-col items-center w-full">
          {gi > 0 && <span className="w-8 h-px bg-studio-border my-1.5" />}
          <span className="text-micro font-bold uppercase tracking-[0.12em] text-ink-faint mb-1 mt-1">
            {group.cap}
          </span>
