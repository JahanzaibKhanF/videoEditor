 ...
 *   2. FabricOverlay     — invisible Fabric.js canvas for drag/resize interaction handles only
 *
 * This is exactly how Adobe Express works:
 * - One canvas that composites everything
 * - A separate transparent interaction layer for UI handles
 */
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.code === "Space" && !isShowProcessedVideo) {
        // Don't intercept spacebar when user is typing in a fabric textbox or input
        const tag = (e.target as HTMLElement)?.tagName;
        if (tag === "INPUT" || tag === "TEXTAREA") return;
        // Fabric textbox editing sets contenteditable on a hidden element
      ...
      >
        {width > 0 && height > 0 && (
          <>
            ...
            {/* Layer 2: Fabric.js — interaction only (drag/resize handles) */}
            {engineReady && (
              <FabricOverlay width={width} height={height} />
            )}
          </>
        )}
