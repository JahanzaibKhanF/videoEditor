 ...
 * one sub-row per active keyframe track. Full-width so the diamonds line up
 * with the clips / ruler / playhead above (NO label column of its own — the
 * prop name floats at the row's left edge).
 *
...
export default function KeyframeLane() {
  ...
  const selKey = sel && trk.find(t => t.prop === sel.prop)?.keys.find(k => k.id === sel.id);
