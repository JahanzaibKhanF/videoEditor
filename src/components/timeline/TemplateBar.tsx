...
import TemplateClipRangeModal from "./TemplateClipRangeModal";
...
export default function TemplateBar() {
  const {
    ...
    setLayerOrder, setClipEffects,
  } = useAppDetailsContext();
  const [rangeModalSlot, setRangeModalSlot] = useState<number | null>(null);
    ...
    >
      ...
      <div className="flex gap-1.5 overflow-x-auto scrollbar-thin flex-1">
        {activeTemplate.slots.map((slot, i) => (
          <div
            key={i}
            onClick={() => setRangeModalSlot(i)}
            title={`Edit ${slot.label} — pick range or replace`}
      ...
      {rangeModalSlot !== null && (
        <TemplateClipRangeModal slotIndex={rangeModalSlot} onClose={() => setRangeModalSlot(null)} />
      )}
    </div>
  );
}
