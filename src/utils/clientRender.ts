...
import { ffmpegColorFilterString } from "./colorAdjustments";
import { hasSpeedRamp, totalSourceConsumed } from "./speedRamp";
import {
...
function canPassThrough(
  ...
  if (hasSpeedRamp(clip)) return false; // pass-through would ignore the ramp entirely
...
/**
 * ffmpeg's `atempo` filter only accepts a single tempo factor in [0.5, 2.0].
 * Speed-ramp audio can need factors well outside that (e.g. 0.35x slow-mo,
 * 3.2x fast motion), so chain multiple atempo stages whose product hits the
 * target — the standard technique for this ffmpeg limitation.
 */
function buildAtempoChain(targetTempo: number): string {
  let t = Math.max(0.05, Math.min(20, targetTempo));
  const stages: number[] = [];
  while (t < 0.5) { stages.push(0.5); t /= 0.5; }
  while (t > 2.0) { stages.push(2.0); t /= 2.0; }
  stages.push(t);
  return stages.map(s => `atempo=${s.toFixed(4)}`).join(",");
}

// NOTE: clip-level effects (shake/wiggle/colorBurst/particles/gradientOverlay,
// see ClipEffectDetails in types.ts), per-property keyframes (KeyframeTrack
// on text/image/clip/blur, see keyframes.ts), AND shape/brush layers
// (ShapeDetails/BrushDetails in types.ts) are rendered by compositeFrame.ts,
// which this FFmpeg fallback path does NOT use — it builds its own independent
// ffmpeg filter_complex graph instead. That means those effects/keyframes/
// shape+brush layers only appear in exports that actually go through the
// WebCodecs path (renderWithWebCodecs in webCodecsRender.ts); if a browser
// falls back to this function, clips still render correctly, just without
// effects, keyframed motion, or any shape/brush layers. Building equivalent
// ffmpeg filters for procedural effects like particles (or arbitrary vector
// shapes/freehand paths) isn't practical, so this is a deliberate, documented
  ...
  try {
    ...
    try {
      ...
      sortedClips.forEach((clip, i) => {
        ...
        // Output duration is always the clip's fixed placement on the
        // master timeline — never affected by speed. For a normal clip
        // this equals endTime-startTime (the trim span) exactly; for a
        // speed-ramped clip it doesn't, because a fixed on-timeline window
        // can consume a different amount of source material.
        const dur = Math.max(0.04, (clip.endPosition ?? 0) - (clip.startPosition ?? 0));
        const ramped = hasSpeedRamp(clip);
        // How much SOURCE material this clip actually consumes. For a
        // ramped clip this is computed from the ramp curve; FFmpeg's
        // setpts is a single linear time-remap though, so a multi-point
        // ramp here only gets its time-weighted AVERAGE speed applied —
        // full per-segment fidelity (true "slow then suddenly fast") is
        // only available via the WebCodecs export path, which renders
        // frame-by-frame and can honor the exact curve. Documented, not
        // hidden: this is the FFmpeg *fallback* used when WebCodecs isn't
        // available.
        const sourceSpan = ramped ? Math.max(0.04, totalSourceConsumed(clip)) : dur;
        const end = start + sourceSpan;
        ...
        const vLabel = `v${i}`;
        // Trim -> (speed remap if ramped) -> scale to clip's rendered
        // size*scale -> place onto a container-sized canvas at (x,y) ->
        // normalize fps/timestamps.
        const speedRemap = ramped ? `,setpts=PTS*${(dur / sourceSpan).toFixed(6)}` : "";
        filterParts.push(
          `[${srcIdx}:v]trim=start=${start}:end=${end},setpts=PTS-STARTPTS${speedRemap},` +
        ...
        // matches the same adjustments applied live in the canvas preview
        // and the WebCodecs export path (see colorAdjustments.ts). Only
        // adds a filter step when the clip actually has non-default values.
        let gradedLabel = `${vLabel}_scaled`;
        const colorFilter = ffmpegColorFilterString(clip.colorAdjustments);
        if (colorFilter) {
          gradedLabel = `${vLabel}_graded`;
          filterParts.push(`[${vLabel}_scaled]${colorFilter}[${gradedLabel}]`);
        }
        ...
        filterParts.push(
          `[${vLabel}_bg][${gradedLabel}]overlay=${x}:${y}:shortest=1,fps=${outFps}[${vLabel}]`
        );
        ...
        if (!muted && inputHasAudio.get(fsName)) {
          ...
          const aLabel = `a${i}`;
          const tempo = sourceSpan / dur; // >1 = speed up audio, <1 = slow down
          const tempoFilter = ramped ? `,${buildAtempoChain(tempo)}` : "";
          filterParts.push(
            `[${srcIdx}:a]atrim=start=${start}:end=${end},asetpts=PTS-STARTPTS${tempoFilter},volume=${vol}[${aLabel}]`
          );
      ...
      if (segLabels.length === 1) {
        ...
        let cumulative = Math.max(0.04, (sortedClips[0].endPosition ?? 0) - (sortedClips[0].startPosition ?? 0));
      ...
      imagesDetails.forEach((img, i) => {
        ...
        const colorFilter = ffmpegColorFilterString(img.colorAdjustments);
        filterParts.push(
          `[${srcIdx}:v]scale=${w}:${h}${colorFilter ? `,${colorFilter}` : ""},format=rgba,colorchannelmixer=aa=${opacity}[${scaledLabel}]`
        );
