...
// Tracks in-flight jobs so RenderingLoader / RenderButton can cancel them.
...
export async function clientRender(
  ...
  try {
    ...
    if (jobState.cancelled) throw new Error("cancelled");
      ...
      textsDetails.forEach((t: TextDetails, i: number) => {
        ...
        switch (anim) {
          ...
          case "blingIn": {
            ...
            // We re-express as fontsize= dynamic value; x/y compensate for growth
    ...
    const cancelled = jobState.cancelled || (err instanceof Error && err.message === "cancelled");
