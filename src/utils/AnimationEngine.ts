...
export function computeAnimState(
  ...
  switch (animation) {
    ...
    case "shake": {
      // Continuous jitter for the whole visible duration (not just an
      // intro) — the "energetic caption" look short-form video apps use.
      s.opacity = interpolate(relFrame, [0, fps * 0.15], [0, 1]);
      s.tx = x + Math.sin(relFrame * 1.8) * 6;
      s.ty = y + Math.cos(relFrame * 2.3) * 3;
      break;
    }
    case "wiggle": {
      // Continuous gentle rotation oscillation for the whole duration.
      s.opacity = interpolate(relFrame, [0, fps * 0.15], [0, 1]);
      s.rotation = Math.sin(relFrame * 0.35) * 6;
      break;
    }
    case "sparkle": {
      // Twinkling pulse — fast opacity/scale flicker layered on top of a
      // slower breathing scale, reads as "sparkly" without a real particle
      // system (which would need its own per-particle simulation state,
      // not just a per-frame pure function like every other animation
      // here).
      s.opacity = interpolate(relFrame, [0, fps * 0.15], [0, 1]) *
        (0.85 + 0.15 * Math.sin(relFrame * 0.9));
      s.scale = 1 + 0.04 * Math.sin(relFrame * 0.9) + 0.015 * Math.sin(relFrame * 3.1);
      break;
    }
    case "rewindIn": {
