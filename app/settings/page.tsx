...
function MotionPresetsSection() {
  const [kind, setKind] = useState<"animation" | "transition">("animation");
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center bg-studio-surface border border-studio-border rounded-lg p-0.5">
            {(["animation", "transition"] as const).map((k) => (
            ...
            return (
              <div key={p.id} className="bg-studio-surface border border-studio-border rounded-xl p-4 flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[10.5px] text-ink-faint font-mono truncate mb-2">{p.preset_json.engineKey}</div>
  ...
  const handleSave = async () => {
    ...
    try {
      const body = {
        name: name.trim(),
        sortOrder,
        presetJson: { engineKey, description: description.trim(), icon },
      };
      const data = preset
